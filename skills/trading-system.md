...
┌────────────┐    ┌──────────────────┐    ┌──────────────────┐
│ 数据部门     │    │  策略部门组长     │    │  回测部门         │
│ data-agent │◄──►│  strategy-director     │    │  backtest-agent  │
│ (行情/交易)  │    │  (7个策略视角)    │    │  (历史验证)       │
└────────────┘    └────────┬─────────┘    └──────────────────┘
                           │ 7票 (BUY/SELL/HOLD)
                           ↓
...
| **strategy-director** | AGT-001 | 策略部门 | **身兼两职**：(1) 中心调度器，每3分钟巡检派发任务；(2) 一人模拟7个策略视角（MACD金叉、均线趋势、布林带、成交量、海龟、技术面综合、主力行为+基本面）各投1票 |
...
| **review-auditor** | RAG-001 | 审核部门 | **一人模拟5个审核视角**（MACD、RSI、布林带、海龟、均线交叉），对每笔已成交交易独立审核（PASS/WARN/FAIL），汇总报告给HR |
...
- **去掉了 scheduler-agent** — 调度职责并入 strategy-director 的第二职责
- **去掉了 watch-agent** — 盯盘职责并入 sentiment-agent + strategy-director 巡检
- **去掉了 auditor-agent** — 审计职责并入 hr-agent（人事决策由HR综合审核报告+胜率数据判断）
- **去掉了 review-02~05** — 全部合并到 review-auditor 身上，由他一人模拟5个审核视角
- **去掉了 strategy-02~08** — 全部合并到 strategy-director 身上，由他一人模拟7个策略视角
- **新增 advertising-agent** — 系统唯一对外通知出口
- **新增 backtest-agent** — 独立回测部门
- **每日自成长** — strategy-director 和 review-auditor 每天自行学习新的策略/审核视角
...
### 5. 启动系统
主入口为 strategy-director 的中心调度循环（每3分钟巡检+派发）和 CEO 的系统巡检（每5分钟）。各 Agent 通过 Kanban 任务编排联动。

## 数据流

```
sentiment-agent 维护股池               → strategy-director 获取股池分析
strategy-director (7票) 投票                 → election-committee 统计
election-committee 投票通过            → execution-agent 风控+下单
execution-agent → data-agent 执行      → 结果返回
data-agent 通知交易完成                → review-auditor 审核
review-auditor (5个视角) 审核报告           → hr-agent 审计
...
- **strategy-director**: 每天0:00搜索新策略，评估是否新增策略视角（AGT-009+），更新到 docs/strategy/learned.md
- **review-auditor**: 每天0:00回顾当天审核交易，学习新审核框架，更新到 docs/review/learned.md
