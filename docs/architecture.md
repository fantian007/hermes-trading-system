...
| **Agent** | 一切决策：风控、投票、淘汰、下单量、买卖时机 | "NVDA 涨 3%，我认为应该投票"、"RAG-0003 40%胜率进影子期" |
...
|---|------|-------|---------|------|---------|-----------|
| 1 | **数据部门** | `data-agent` | — | 1 | 工具人，你问我答 | IT 运维 |
| 2 | **选股部门** | `strategy-01~04` | AGT-0001~0004 | 4 | 技术宅，发现异动 | 分析师 |
...
```
角色定位：系统唯一的行情数据接口
工作方式：被动响应，其他部门通过自然语言请求数据
```

| 请求类型 | 实际命令 |
|---------|---------|
| "帮我查 NVDA 报价" | `data-service.ts --type quote --symbol NVDA.US` |
...
重要规则：**其他部门不能直接查行情数据，必须通过 data-agent。**
...
| 工号 | 策略 | 来源 | 核心逻辑 |
|------|------|------|---------|
| AGT-0001 | 均线交叉 | 《股市趋势技术分析》 | MA5 上穿 MA20 买入，下穿卖出，成交量确认 |
| AGT-0002 | MACD | 技术分析 | DIF 上穿 DEA 买入，柱状图背离确认 |
| AGT-0003 | RSI | 技术分析 | RSI<30 超卖买入，RSI>70 超买卖出 |
| AGT-0004 | 布林带 | 技术分析 | 触及下轨买入，触及上轨卖出，带宽收缩预示突破 |
...
| 工号 | 名称 | 审核框架 | 审核视角 |
|------|------|---------|---------|
| RAG-0001 | 均线交叉审核官 | MA5/MA20 位置关系 | "买入时均线是否形成有效金叉?" |
| RAG-0002 | MACD审核官 | MACD柱状图+信号线 | "DIF/DEA 是否支持该方向?" |
| RAG-0003 | RSI审核官 | 超买/超卖区域 | "入场时 RSI 是否在合理区间?" |
| RAG-0004 | 布林带审核官 | 轨道位置+带宽 | "价格在布林带中的位置合适吗?" |
| RAG-0005 | 海龟交易审核官 | 唐奇安通道 | "是否突破了合理通道?" |
...
职责：
1. 收到选举委员会的 BUY/SELL 指令后执行下单
2. **自行做风控判断**（不再是代码自动检查）
3. 持续监控持仓，发现卖出信号报告选举委员会

风控底线（存储在配置中，Agent 自行决定是否严格遵守）：
- 单票仓位上限：20%
- 日交易次数上限：10 次
- 最低现金保留：10%
- 单笔最大亏损：5%
...
erDiagram
    ...
    agents {
        string agent_id PK "工号 AGT-0001"
