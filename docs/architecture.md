...
```
|策略Agent ──找 data-agent 要数据──→ 行情 JSON ──自己分析→ 发起投票请求给选举委员会
...
| 2 | **策略部门** | `strategy-01~07` | AGT-001~007 | 7 | 独立分析师，自主排班 | 分析师/交易员 |
| 3 | **选举委员会** | `election-committee` | — | 1 | 最终拍板人 | 投资总监 |
| 4 | **审核部门** | `review-01~06` | RAG-001~006 | 6 | 事后诸葛亮 | 风控审计 |
...
组长 AGT-001 只负责对外接口和人事管理，**不参与具体策略分析和投票**。组员任务过重时找组长提扩招需求，组长评估后找 HR 安排。
...
输出：**审核报告**（含技术审核结论 + Agent 绩效评估），提交给 HR 部门。
