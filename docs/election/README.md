...
| 职责 | 说明 |
|------|------|
|| **召集投票** | 收到策略 Agent 请求后，向全体 strategy-director（组长一人投7票）逐一征集意见（BUY/SELL/HOLD + 置信度） |
...
```
策略部门(strategy-director（组长一人投7票）)──发起投票请求──→ 选举委员会
                                           │
                                         计票
                                           │
...
| 方向 | 对端 | 触发条件 | 通信方式 |
|------|------|---------|---------|
| 入 | strategy-director（组长一人投7票） | 策略 Agent（组长一人）发起投票请求 | 自然语言（含 round_id） |
...
3. ELC-001 召集全体策略Agent
   向 strategy-director（组长一人投7票）逐一征集意见（BUY/SELL/HOLD + 置信度）
                │
4. 收集完成，跑聚合脚本
...
| 规则 | 说明 |
|------|------|
| **发起方** | 仅策略 Agent（strategy-director，组长一人）可发起 |
| **投票人** | 策略组长一人（strategy-director），一人投7票 |
...
```
ELC-001 自愈 → strategy-director（组长）→ CEO → 飞书通知用户（仅无法解决时）
```
