...
| 职责 | 说明 |
|------|------|
|| **召集投票** | 收到策略 Agent 请求后，向全体策略分析官（strategy-02~07）逐一征集意见（BUY/SELL/HOLD + 置信度） |
...
| 方向 | 对端 | 触发条件 | 通信方式 |
|------|------|---------|---------|
| 入 | strategy-02~07（策略分析官） | 策略 Agent 发起投票请求 | 自然语言（含 round_id） |
...
| 规则 | 说明 |
|------|------|
| **发起方** | 任何策略分析官（strategy-02~07）可发起 |
| **投票人** | 策略分析官（strategy-02~07），每人独立投票 |
