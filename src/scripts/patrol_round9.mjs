...
// 1. PASSED 但无trade的投票
const dead = db.prepare("SELECT round_id, symbol, direction, status, created_at FROM election_rounds WHERE status='PASSED' AND resulted_trade_id IS NULL ORDER BY created_at DESC").all();
...
const recent = db.prepare("SELECT round_id, symbol, direction, status, final_decision, resulted_trade_id, created_at FROM election_rounds ORDER BY created_at DESC LIMIT 10").all();
console.log('\n=== 最近10轮选举 ===');
